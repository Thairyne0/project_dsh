...
class AddStoreCategoryToStorePage extends StatefulWidget {
  const AddStoreCategoryToStorePage({super.key, required this.id});

  final String id;

  @override
  createState() => AddStoreCategoryToStorePageState();
}

class AddStoreCategoryToStorePageState extends State<AddStoreCategoryToStorePage> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return ViewModelBuilder<StoreViewModel>.reactive(
        viewModelBuilder: () => StoreViewModel(context, VMType.other, widget.id),
        onViewModelReady: (vm) async => await vm.initialize(pageActions: [
          PageAction(
              title: "Salva",
              isMain: true,
              onTap: () async {
                if (_formKey.currentState!.validate() && vm.selectedStoreCategories.isNotEmpty) {
                        ...
                        ResponsiveGrid(
                          ...
                          children: [
                            ...
                            ResponsiveGridItem(
                              ...
                              child: Column(
                                children: [
                                  CLDropdown<StoreCategory>.multipleAsync(
                                    searchCallback: vm.getAllStoreCategoryByStore,
                                    searchColumn: "name",
                                    valueToShow: (item) {
                                      return item.modelIdentifier;
                                    },
                                    onSelectItems: (items) {
                                      vm.selectedStoreCategories = items;
                                    },
